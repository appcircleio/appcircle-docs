...
- [Custom Roles (Beta)](/account/my-organization/profile-and-team/custom-roles) (Granular scope-based role management) has been introduced, allowing permissions to be granted per action instead of per module. <AccountBadge/> <CloudBadge/> <SelfHostedBadge/>
- [Scheduled builds (Beta)](/build/build-process-management/build-manually-or-with-triggers#scheduled-builds-beta) can now be defined, viewed and edited from the build profile Triggers tab. <BuildBadge/> <CloudBadge/> <SelfHostedBadge/>
- An optional expiration date can now be entered for external Personal Access Tokens in the connection form. <BuildBadge/> <CloudBadge/> <SelfHostedBadge/>
...
- A Fixed version option has been added to Re-sign, allowing a version to be entered manually instead of being fetched from a store. <SigningIdentitiesBadge/> <CloudBadge/> <SelfHostedBadge/>
...
- A new notification event has been added for version deletion in the Publish to Stores, Testing Distribution and Enterprise App Store modules, so connected notification channels are triggered when a version is removed. <AccountBadge/> <CloudBadge/> <SelfHostedBadge/>
- The LDAP attribute used for user email can now be configured instead of being fixed. <AccountBadge/> <CloudBadge/> <SelfHostedBadge/>
- Team management actions are now blocked when LDAP Mapping is enabled: adding a new user, editing roles, removing a user and leaving the organization are disabled, and admins are directed to the LDAP Mapping page instead. <AccountBadge/> <SelfHostedBadge/>
- LDAP authentication and authorization are now managed from separate screens under Self-Hosted Settings. A new LDAP configuration can be created from scratch or from an existing one, and group and role mappers have been moved from the LDAP configuration details page to a dedicated Manage Authorization page. <AccountBadge/> <SelfHostedBadge/>
- Google Play release names now include the version code alongside the version name for easier identification. <PublishBadge/> <CloudBadge/> <SelfHostedBadge/>
- Downloaded CSR files now use descriptive file names instead of `download.csr`. <ReportsBadge/> <CloudBadge/> <SelfHostedBadge/>
- The Device column in the Testing Distribution downloads report has been renamed to Platform to better reflect its content. <ReportsBadge/> <CloudBadge/> <SelfHostedBadge/>
- The certificate renewal screen descriptions have been updated to correctly reflect both revoke enabled and revoke disabled cases. <SigningIdentitiesBadge/> <CloudBadge/> <SelfHostedBadge/>
- Added a status reason field under Profile Details in Webhook Details to help troubleshoot failed webhook events. <BuildBadge/> <CloudBadge/> <SelfHostedBadge/>
...
- The Self-Hosted Runners page has been streamlined: the Runner and Pool Name columns are merged into a single `Runner / Pool Name` column where the runner name links to the runner details, network availability and execution state are combined into a single `Status / State` column, and the pool deletion action has been removed to prevent accidental misconfiguration. <InfrastructureBadge/> <SelfHostedBadge/>
- A new macOS-based build stack (Tahoe `26.6.2`) is released for the self-hosted Appcircle environments, which has the latest GA release of [Xcode 27.0](https://developer.apple.com/documentation/xcode-release-notes/xcode-27-release-notes) (`27A266a`) installed along with Xcode 26.6 (`17F113`), plus up-to-date [build tools](/infrastructure/ios-build-infrastructure#ios-build-environment) for iOS and Android builds. Follow [the setup guide](/self-hosted-appcircle/self-hosted-runner/runner-vm-setup#download-macos-vm) for installation instructions. <InfrastructureBadge/> <SelfHostedBadge/>
...
- The Git Clone step has two new optional inputs: Fetch Depth and Fetch Jobs. Fetch Depth limits how many commits are fetched, so a build that pins a specific commit no longer fetches the full history of its branch. This shortens the clone step considerably on large repositories, and the requested commit is still guaranteed to be checked out: if it turns out to be older than the given depth, the remaining history is fetched before the checkout. The same depth also applies to submodules. Fetch Jobs sets how many parallel jobs Git uses while fetching, including the recursive submodule update. <BuildIntegrationsBadge/> <CloudBadge/> <SelfHostedBadge/>
- The Android Badge step now supports adaptive and WebP launcher icons in addition to legacy PNG icons. <BuildIntegrationsBadge/> <CloudBadge/> <SelfHostedBadge/>
...
- An issue has been fixed where the component version text displayed an incorrect value on Publish step logs. <PublishBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where a Stop button was still displayed on a completed publish flow. <PublishBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where the staged rollout percentage could not be set below 10% in Distribute to Track. <PublishIntegrationsBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where build logs displayed "waiting for other build" until the page was refreshed even though the build had already started. <BuildBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed in the SSH key auto-fill form layout. <BuildBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where the commit status remained "pending / in progress" on the repository provider when a queued build was canceled. <BuildBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where the "Build Success at" label was missing on some branches in the branch list despite successful builds. <BuildBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where the GitLab commit status link pointed to the wrong branch for merge commits. <BuildBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where the triggering user was reported incorrectly for some builds. <BuildBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where adding more than one custom script step to a build workflow caused an invalid character error on save. <BuildIntegrationsBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where refreshing the browser in the Enterprise App Store caused a package or bundle ID mismatch warning on all binaries and blocked beta and live publishing. <EnterpriseStoreBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where existing testers could not be selected manually on a Testing Distribution profile. <DistributionBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where the "Shared time" information on distribution profile cards was not updated after an automatic distribution from a build workflow. <DistributionBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where the Yesterday filter on report pages returned the same records as Today. <ReportsBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where filtering by Profile Name in activity logs broke the general filter list. <ReportsBadge/> <CloudBadge/> <SelfHostedBadge/>
- An issue has been fixed where pending invite actions (re-sending an invite, updating its roles and deleting it) remained enabled even though organization membership was managed by LDAP Mapping. <AccountBadge/> <SelfHostedBadge/>
...
- [Auth Activity Log](/docs/account/my-organization/activity-log/auth-activity-log.md) support has been added to the Organization module, providing visibility into user login, logout, and organization switch events. <AccountBadge/> <CloudBadge/> <SelfHostedBadge/>
...
- Publish to Stores module support has been added to Appcircle's marketplace plugins, enabling the Publish flow to be triggered directly from your CI/CD pipeline alongside the existing Testing Distribution and Enterprise App Store modules. This is now available on the [Fastlane](/marketplace/fastlane/publish-to-stores), [GitHub Actions](/marketplace/github-marketplace/publish-to-stores), [Jenkins](/marketplace/jenkins/publish-to-stores), and [Visual Studio Marketplace](/marketplace/visual-studio-marketplace/publish-to-stores) plugins. <BestPracticesBadge/> <PublishBadge/> <CloudBadge/> <SelfHostedBadge/>
...
- A new OAuth 2 integration, [Azure DevOps Entra ID](/build/manage-the-connections/connection-guides/connecting-to-azure#connecting-with-azure-devops-entra-id), has been added to the UI, enabling authorization for Azure DevOps on Build Profiles. <BuildBadge/> <CloudBadge/>
...
- [App Center iOS Distribution](/workflows/ios-specific-workflow-steps/appcenter-ios-distribution) workflow step added. <CloudBadge/> <SelfHostedBadge/>

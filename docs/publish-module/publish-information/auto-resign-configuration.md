...
Because an Auto Re-sign configuration is reused for **every** binary uploaded to the profile, selecting **Fixed** here signs every incoming binary with the exact same version or build number until you update the configuration or switch to a different source. This is different from the offset-based sources, which recalculate a new value for each incoming binary. Use **Fixed** in Auto Re-sign only when you intentionally want to pin a channel to a single version/build number.
...
- **Build Number Source**: The defined base build number will be used for versioning during the re-signing process. **App Store**, **TestFlight**, **Uploaded Binary**, and **Fixed** are available options.
  ...
  - **Fixed**: Every incoming binary will be signed with the exact build number you enter, without referencing the App Store, TestFlight, or the uploaded binary.
...
> ⚠️ Note: When **Build Number Source** is set to **Fixed**, the offset field above is replaced with a **Build Number** text input. Enter the exact build number that incoming binaries should be signed with. This value must be provided whenever **Update Build Number** is enabled and the source is **Fixed**; leaving it empty will cause the re-signing process to fail. The saved value is shown here again whenever you reopen this configuration.
...
- **Version Number**: The defined base version number will be used for versioning during the re-signing process. **App Store**, **TestFlight**, **Uploaded Binary**, and **Fixed** are available options
  ...
  - **Fixed**: Every incoming binary will be signed with the exact version number you enter, without referencing the App Store, TestFlight, or the uploaded binary.
...
> ⚠️ Note: When **Version Number Source** is set to **Fixed**, both the **Offset** and **Increment Strategy** controls above are hidden and replaced with a **Version Number** text input. Enter the exact version number that incoming binaries should be signed with. This value must be provided whenever **Update Version Number** is enabled and the source is **Fixed**; leaving it empty will cause the re-signing process to fail. The saved value is shown here again whenever you reopen this configuration.
...
Because an Auto Re-sign configuration is reused for **every** binary uploaded to the profile, selecting **Fixed** here signs every incoming binary with the exact same version code or version name until you update the configuration or switch to a different source. This is different from the offset-based sources, which recalculate a new value for each incoming binary. Use **Fixed** in Auto Re-sign only when you intentionally want to pin a channel to a single version code/name.
...
- **Version Code Source**: The defined base version code will be used for versioning during the re-signing process. **Google Play**, **Uploaded Binary**, and **Fixed** are available options.
  ...
  - **Fixed**: Every incoming binary will be signed with the exact version code you enter, without referencing Google Play or the uploaded binary.
...
> ⚠️ Note: When **Version Code Source** is set to **Fixed**, the **Version Code Offset** field above is replaced with a **Version Code** text input. Enter the exact version code that incoming binaries should be signed with. This value must be provided whenever **Update Version Code** is enabled and the source is **Fixed**; leaving it empty will cause the re-signing process to fail. The saved value is shown here again whenever you reopen this configuration.
...
- **Version Number/Version Name Source**: The defined base version name will be used for versioning during the re-signing process. **Google Play**, **Uploaded Binary**, and **Fixed** are available options
  ...
  - **Fixed**: Every incoming binary will be signed with the exact version name you enter, without referencing Google Play or the uploaded binary.
...
> ⚠️ Note: When **Version Number/Version Name Source** is set to **Fixed**, both the **Version Name Offset** and **Increment Strategy** controls above are hidden and replaced with a **Version Name** text input. Enter the exact version name that incoming binaries should be signed with. This value must be provided whenever **Update Version Name** is enabled and the source is **Fixed**; leaving it empty will cause the re-signing process to fail. The saved value is shown here again whenever you reopen this configuration.
